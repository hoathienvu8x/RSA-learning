...
/**
 * Load a bignum with a random value of exactly nbits bits.
 * The highest bit is always set to 1 to guarantee the bit length.
 */
void bignum_frombits(bignum *b, int nbits) {
  ...
  for (int i = 0; i < nwords; i++) {
    b->data[i] = ((word)rand() << 16) | ((word)rand() & 0xFFFF);
  }
  // Zero upper bits in the highest word
  int upper = nbits % 32;
  if (upper != 0) {
    word mask = (1UL << upper) - 1;
    b->data[nwords - 1] &= mask;
    b->data[nwords - 1] |= (1UL << (upper - 1)); // Ensure high bit set
  } else {
    b->data[nwords - 1] |= (1UL << 31); // Ensure high bit set
  }
  // Remove leading zero words (shouldn't happen, but just in case)
...
void randPrimeBits(int nbits, bignum* result) {
  do {
    bignum_frombits(result, nbits);
    // Ensure it's odd
    result->data[0] |= 1;
  } while (!probablePrime(result, ACCURACY));
}
/**
